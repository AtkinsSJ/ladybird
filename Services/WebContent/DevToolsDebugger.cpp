...
ErrorOr<void> DevToolsDebugger::set_breakpoint(PageClient& page, WebView::DebuggerBreakpointLocation location, WebView::DebuggerBreakpointOptions options)
{
    if (auto registrations = m_breakpoints.find(page.id()); registrations != m_breakpoints.end()) {
        if (auto registration = registrations->value.find_if([&](auto const& registration) { return registration.location == location; }); registration != registrations->value.end()) {
...
PageClient* DevToolsDebugger::paused_page_client() const
{
    ...
    auto* window = as_if<Web::HTML::Window>(vm.current_realm()->global_object());
    if (!window)
        return nullptr;
    ...
    if (result.is_throw_completion()) {
        ...
        if (!value.is_object()) {
            ...
            if (length.is_throw_completion()) {
                ...
                arguments.ensure_capacity(length.value());
                for (u64 i = 0; i < length.value(); ++i) {
...
void DevToolsDebugger::handle_pause(JS::Debugger::PauseInfo const& pause)
{
    ...
    if ((pause.reason == JS::Debugger::PauseReason::DebuggerStatement && !configuration.should_pause_on_debugger_statement)
        ...
        || (pause.reason == JS::Debugger::PauseReason::Exception
            && (configuration.skip_breakpoints || !configuration.pause_on_exceptions
