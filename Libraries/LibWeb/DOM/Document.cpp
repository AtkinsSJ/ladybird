...
Document::IndicatedPart Document::determine_the_indicated_part()
{
    ...
    if (text_directives.has_value() && !text_directives->is_empty()) {
        // 2.1. Let ranges be a list that is the result of running the invoke text directives steps
        ...
        // 2.2. If ranges is non-empty, then:
        if (!ranges.is_empty()) {
            // 2.2.1. Let firstRange be the first item of ranges.
            auto first_range = ranges.first();

            // 2.2.2. Visually indicate each range in ranges in an implementation-defined way. The
            ...
            // 2.2.3. Set firstRange as document's indicated part, return.
...
void Document::scroll_to_the_fragment(bool allow_text_directive_scroll)
{
    ...
    // 3. Otherwise:
        ...
        if (is_text_directive) {
            // 4.1. If allow text directive scroll is false, return.
        ...
        auto run_the_ancestor_revealing_algorithm = [&] {
            ...
            for (size_t i = 0; i < ancestors_to_reveal.size(); ++i) {
                ...
                // 4.2. If revealType is "until-found":
                if (reveal_type == RevealType::UntilFound) {
                    ...
                    // 4.2.2. Fire an event named beforematch at ancestorToReveal with the bubbles
                    ...
                    // 4.2.4. If ancestorToReveal's hidden attribute is not in the Hidden Until
                    // Found state, then return.
                    ...
                    // 4.2.5. Remove the hidden attribute from ancestorToReveal.
                ...
                // 4.3. Otherwise:
                    ...
                    // 4.3.3. Set ancestorToReveal's open attribute to the empty string.
...
void Document::try_to_scroll_to_the_fragment(bool allow_text_directive_scroll)
{
    ...
    // the fragment, then:
    if (!is_fully_active()) {
        // 1.1. Set pending text directives to null.
        clear_pending_text_directives();

        // 1.2. Abort these steps.
        return;
    }
