...
Counter& CountersSet::instantiate_a_counter(FlyString name, DOM::ElementReference const& element, bool reversed, Optional<CounterValue> value)
{
...
void CountersSet::set_a_counter(FlyString name, DOM::ElementReference const& element, CounterValue value)
{
...
void CountersSet::increment_a_counter(FlyString name, DOM::ElementReference const& element, CounterValue amount)
{
    if (auto existing_counter = last_counter_with_name(name); existing_counter.has_value()) {
...
Optional<Counter&> CountersSet::counter_with_same_name_and_creator(FlyString const& name, DOM::ElementReference const& element)
{
...
void resolve_counters(DOM::ElementReference& element_reference)
{
    dbgln("Resolve counters: {}", element_reference.debug_description());
...
void inherit_counters(DOM::ElementReference& element_reference)
{
    dbgln("Inherit counters: {}", element_reference.debug_description());
