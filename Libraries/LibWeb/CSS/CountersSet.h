...
struct Counter {
    FlyString name;
    DOM::ElementReference originating_element; // "creator"
    bool reversed { false };
...
public:
    ...
    Counter& instantiate_a_counter(FlyString name, DOM::ElementReference const&, bool reversed, Optional<CounterValue>);
    void set_a_counter(FlyString name, DOM::ElementReference const&, CounterValue value);
    void increment_a_counter(FlyString name, DOM::ElementReference const&, CounterValue amount);
    void append_copy(Counter const&);

    Optional<Counter&> last_counter_with_name(FlyString const& name);
    Optional<Counter&> counter_with_same_name_and_creator(FlyString const& name, DOM::ElementReference const&);
...
void resolve_counters(DOM::ElementReference&);
void inherit_counters(DOM::ElementReference&);
