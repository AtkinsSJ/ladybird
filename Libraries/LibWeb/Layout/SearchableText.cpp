...
SearchableText SearchableText::collect(Viewport& viewport)
{
    ...
    auto flush_block = [&] {
        if (!builder.is_empty())
            blocks.append({ builder.to_string(), move(segments) });
    ...
    auto append_code_unit = [&](DOM::Text& dom_node, char16_t code_unit, size_t dom_offset, Searchability searchability) {
        if (current_dom_node != &dom_node || dom_offset != expected_dom_offset) {
            segments.empend(dom_node, builder_length_in_code_units, dom_offset, searchability, next_boundary, dom_node.is_inert());
    ...
    auto append_visible_text_node = [&](DOM::Text& text, Searchability searchability) {
        ...
        auto const append_text = [&](Utf16View text_to_append, size_t dom_start_offset) {
            ...
            for (size_t i = 0; i < text_to_append.length_in_code_units(); ++i) {
                auto const code_unit = text_to_append.code_unit_at(i);
                ...
                if (pending_space_dom_node) {
                    if (!segments.is_empty())
                        append_code_unit(*pending_space_dom_node, ' ', pending_space_dom_offset, pending_space_searchability);
                    pending_space_dom_node = nullptr;
                }

                append_code_unit(text, code_unit, dom_offset, searchability);
            }
        };

        if (auto rendered_text = rendered_text_nodes.get(&text); rendered_text.has_value()) {
            for (auto const* text_node : *rendered_text)
                append_text(text_node->text_for_rendering().utf16_view(), text_node->dom_start_offset());
        } else {
    ...
    if (contains_inert_text) {
        ...
        auto flush_find_in_page_block = [&] {
            if (!find_in_page_builder.is_empty())
                filtered_blocks.append({ find_in_page_builder.to_string(), move(find_in_page_segments) });
        ...
        auto append_find_in_page_code_unit = [&](DOM::Text& dom_node, char16_t code_unit, size_t dom_offset, Searchability searchability) {
            if (find_in_page_current_dom_node != &dom_node || dom_offset != find_in_page_expected_dom_offset) {
                find_in_page_segments.empend(dom_node, find_in_page_builder_length_in_code_units, dom_offset, searchability, find_in_page_next_boundary, false);
            ...
            find_in_page_expected_dom_offset = dom_offset + 1;
        };
                ...
                for (size_t offset = segment.start_offset; offset < segment_end; ++offset) {
                    ...
                    if (should_collapse && is_ascii_space(code_unit) && !preserve_line_break) {
                        if (!find_in_page_pending_space_dom_node) {
                            ...
                            find_in_page_pending_space_dom_offset = dom_offset;
                    ...
                    if (find_in_page_pending_space_dom_node) {
                        if (!find_in_page_segments.is_empty())
                            append_find_in_page_code_unit(*find_in_page_pending_space_dom_node, ' ', find_in_page_pending_space_dom_offset, find_in_page_pending_space_searchability);
                        find_in_page_pending_space_dom_node = nullptr;
                    }

                    append_find_in_page_code_unit(*dom_node, code_unit, dom_offset, segment.searchability);
                }
            }
...
Optional<GC::Ref<DOM::Range>> SearchableText::range_from_offsets(Block const& block, size_t start_offset, size_t end_offset) const
{
    ...
    for (; segment_index < block.segments.size() - 1 && start_offset > block.segments[segment_index + 1].start_offset; ++segment_index)
        start_segment = &block.segments[segment_index + 1];

    auto start_node = start_segment->dom_node.ptr();
    if (!start_node)
        return {};
    auto const start_dom_offset = start_offset - start_segment->start_offset + start_segment->dom_offset_within_node;

    auto const* end_segment = start_segment;
    for (; segment_index < block.segments.size() - 1 && end_offset > block.segments[segment_index + 1].start_offset; ++segment_index)
        end_segment = &block.segments[segment_index + 1];

    auto end_node = end_segment->dom_node.ptr();
    if (!end_node)
        return {};
    auto const end_dom_offset = end_offset - end_segment->start_offset + end_segment->dom_offset_within_node;
