...
JS::ThrowCompletionOr<void> CustomElementRegistry::define(String const& name, WebIDL::CallbackType* constructor, ElementDefinitionOptions options)
{
    ...
    //     upgrade particular elements within a document given document and localName.
    if (m_is_scoped) {
...
void CustomElementRegistry::initialize_for_bindings(GC::Ref<DOM::Node> root)
{
    ...
    root->for_each_in_inclusive_subtree([this](auto& inclusive_descendant) {
        if (auto* element = as_if<DOM::Element>(inclusive_descendant); element && element->custom_element_registry() == nullptr) {
            ...
            //    document set.
            if (m_is_scoped)
                m_scoped_documents.set(element->document());
        }
        return TraversalDecision::Continue;
    });
}
...
GC::Ptr<CustomElementRegistry> look_up_a_custom_element_registry(DOM::Node const& node)
{
    ...
    // 3. Return node's custom element registry.
    // FIXME: This gets called on DocumentFragments! https://github.com/whatwg/html/issues/11227
    //        So, check for Document specifically, and return null otherwise.
