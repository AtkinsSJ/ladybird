...
static TrackEvent::TrackTypeInternal to_track_type_internal(TrackEventInit::TrackType track_type)
{
    if (!track_type.has_value())
        return Empty {};

    return track_type->visit(
        [](GC::Root<VideoTrack> const& root) -> TrackEvent::TrackTypeInternal { return GC::Ref { *root }; },
        [](GC::Root<AudioTrack> const& root) -> TrackEvent::TrackTypeInternal { return GC::Ref { *root }; },
        [](GC::Root<TextTrack> const& root) -> TrackEvent::TrackTypeInternal { return GC::Ref { *root }; });
}
