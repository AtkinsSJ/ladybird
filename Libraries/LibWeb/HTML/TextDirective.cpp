...
GC::RootVector<GC::Ref<DOM::Range>> invoke_text_directives(Vector<TextDirective> const& text_directives, DOM::Document& document)
{
    ...
    for (auto const& directive : text_directives) {
        // 2.1. If the result of running find a range from a text directive given directive and
    ...
    for (size_t segment_index = 0; segment_index < block.segments.size(); ++segment_index) {
        ...
        if (node_end > index) {
            ...
            return DOM::BoundaryPoint {
                *node,
                static_cast<WebIDL::UnsignedLong>(segment.dom_offset_within_node + index - counted),
            };
        }
...
Optional<GC::Ref<DOM::Range>> find_a_range_from_a_text_directive(TextDirective const& parsed_values, DOM::Document& document)
{
    ...
    while (!search_range.collapsed()) {
        ...
        if (parsed_values.prefix.has_value()) {
            // 2.2.1. Let prefixMatch be the the result of running the find a string in range steps
            ...
            // 2.2.3. Set searchRange's start to the first boundary point after prefixMatch's start.
            search_range.start = first_boundary_point_after(prefix_match->start);

            // 2.2.4. Let matchRange be a range whose start is prefixMatch's end and end is
            // searchRange's end.
            ...
            // 2.2.8. Let mustEndAtWordBoundary be true if parsedValues's end is non-null or
            ...
            // 2.2.9. Set potentialMatch to the result of running find a string in range with query
            ...
            // 2.2.10. If potentialMatch is null, continue.
        ...
        // 2.3. Otherwise:
            ...
            // 2.3.1. Let mustEndAtWordBoundary be true if parsedValues's end is non-null or
            // parsedValues's suffix is null, false otherwise.
            auto must_end_at_word_boundary = parsed_values.end.has_value() || !parsed_values.suffix.has_value();

            // 2.3.2. Set potentialMatch to the result of running find a string in range with query
            ...
            // 2.3.4. Set searchRange's start to the first boundary point after potentialMatch's
            // start.
            search_range.start = first_boundary_point_after(potential_match->start);
        }

        // 2.4. Let rangeEndSearchRange be a range whose start is potentialMatch's end and whose end
        ...
        while (!range_end_search_range.collapsed()) {
            ...
            if (parsed_values.end.has_value()) {
                // 2.5.1.1. Let mustEndAtWordBoundary be true if parsedValues's suffix is null,
                // false otherwise.
                auto must_end_at_word_boundary = !parsed_values.suffix.has_value();

                // 2.5.1.2. Let endMatch be the result of running find a string in range with query
            ...
            // 2.5.4. Let suffixRange be a range with start equal to potentialMatch's end and end
            ...
            // 2.5.6. Let suffixMatch be result of running find a string in range with query suffix,
            ...
            // 2.5.7. If suffixMatch is non-null, return potentialMatch.
            if (suffix_match.has_value())
                return create_range(*potential_match);

            // 2.5.8. If parsedValues's end item is null and suffixMatch is null, then break.
            if (!parsed_values.end.has_value())
                break;

            // 2.5.9. Set rangeEndSearchRange's start to potentialMatch's end.
        ...
        if (range_end_search_range.collapsed()) {
            // 2.6.1. Assert: parsedValues's end item is non-null.
