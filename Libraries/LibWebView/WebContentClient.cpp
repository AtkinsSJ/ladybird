...
void WebContentClient::did_get_debugger_environments(u64 page_id, u64 request_id, Vector<DebuggerEnvironment> environments)
{
    if (auto view = view_for_page_id(page_id); view.has_value()) {
...
void WebContentClient::did_evaluate_javascript_in_debugger_frame(u64 page_id, u64 request_id, Optional<String> error, DebuggerEvaluationResult result)
{
    if (auto view = view_for_page_id(page_id); view.has_value()) {
        ...
        if (!callback.has_value())
            return;
        if (error.has_value())
            (*callback)(Error::from_string_literal("Unable to evaluate in paused frame"));
        else
...
void WebContentClient::did_get_debugger_object_properties(u64 page_id, u64 request_id, Optional<String> error, DebuggerObjectProperties properties)
{
    if (auto view = view_for_page_id(page_id); view.has_value()) {
        ...
        if (!callback.has_value())
            return;
        if (error.has_value())
            (*callback)(Error::from_string_literal("Unable to inspect paused object"));
        else
