...
void Debugger::continue_execution(ResumeMode mode)
{
    ...
    if (mode == ResumeMode::Continue) {
        ...
        m_step_state = StepState {
            .mode = mode,
            .execution_context = m_paused_execution_context,
...
bool Debugger::should_pause_for_step(Bytecode::Executable const& executable, u32 bytecode_offset) const
{
    ...
    vm.for_each_execution_context_top_to_bottom([&](ExecutionContext const& context) {
        if (&context == state.execution_context) {
    ...
    case ResumeMode::StepInto:
        return current_context != state.execution_context || has_moved();
    case ResumeMode::StepOver:
        if (current_context == state.execution_context)
            return has_moved();
...
bool Debugger::should_pause_on_exception(Bytecode::Executable& executable, u32 bytecode_offset, Value exception, bool exception_will_be_caught)
{
